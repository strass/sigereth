...
export default (
  ...
  const value = useMemo(() => {
    ...
    if (
      ...
      console.warn(
        'Tried to get permissions for anonymous or unauthenticated user'
      );
      return {
        isGameOwner: false,
        isGamePlayer: false,
    ...
    const isResourceOwner =
      resource && user && resource.data.owner && user.ref.isEqual(resource.data.owner);
    return {
      isGameOwner,
      isGamePlayer:
        game && user && some(game.data.players, p => user.ref.isEqual(p)),
      isResourceOwner,
